...
std::vector<int> HandSearch::reevaluateHypothesesMC(
...
#pragma omp parallel for num_threads(params_.num_threads_)
  ...
  for (int i = 0; i < grasps.size(); i++) {
    ...
    if (kdtree.radiusSearch(sample_pcl, nn_radius_, nn_indices, nn_dists) > 0) {
      ...
      nn_points.setPoints(nn_points.getPoints() - sample.replicate(1, nn_points.size()));
...
int HandSearch::labelHypothesis(const util::PointList &point_list,
  ...
  int antipodal_result = antipodal_->evaluateGrasp(
      point_list_learning, 0.01, finger_hand.getLateralAxis(),
