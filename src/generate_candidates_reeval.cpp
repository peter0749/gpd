...
int DoMain(int argc, char *argv[]) {
  ...
  candidate::CandidatesGenerator candidates_generator(generator_params,
                                                      hand_search_params);
  candidate::HandSearch hand_search(hand_search_params);
  ...
  std::vector<int> labels = hand_search.reevaluateHypothesesMC(cloud, candidates, plot_candidates);
  std::vector<size_t> full_index, half_index;
  std::vector<std::unique_ptr<candidate::Hand>> full_grasps, half_grasps;
  int invalid_grasps = 0;
  for (size_t i=0; i<labels.size(); ++i) {
    if (labels[i]>0) {
      if (labels[i]==1) {
          half_index.push_back(i);
      } else if (labels[i]==2) {
  ...
  std::cout << "#Full: " << full_index.size() << '\n' << "#Half: " << half_index.size() << '\n' << "#Invalid: " << invalid_grasps << std::endl;

  std::random_shuffle(full_index.begin(), full_index.end());
  std::random_shuffle(half_index.begin(), half_index.end());

  for (size_t i=0; i<std::min((size_t)100, full_index.size()); ++i) full_grasps.push_back(std::move(candidates[full_index[i]]));
  for (size_t i=0; i<std::min((size_t)100, half_index.size()); ++i) half_grasps.push_back(std::move(candidates[half_index[i]]));

  util::Plot plotter_half(hand_axes.size(), num_orientations);
  plotter_half.plotFingers3D(half_grasps, cloud.getCloudOriginal(),
      "Grasp candidates (half)", hand_geom);
  util::Plot plotter_full(hand_axes.size(), num_orientations);
  plotter_full.plotFingers3D(full_grasps, cloud.getCloudOriginal(),
      "Grasp candidates (full)", hand_geom);
